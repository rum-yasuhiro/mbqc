...
class MBQCU3Gate(MBQCSingleGate):
    """general one-qubit gate."""

    def __init__(
        self, theta, phi, lam, input_state: QuantumRegister = None,
    ):
        """Create new S gate."""
        super().__init__(name="u3", input_state=input_state)

        self.theta = theta
        self.phi = phi
        self.lam = lam

        self._cr02 = ClassicalRegister(2)
        self._cr13 = ClassicalRegister(2)
    ...
    def _set_meas_angle(self):
        ...
        set_angle_circuit.ry(np.pi / 2, self._qr[1])
    ...
    def _measurement(self):
        meas_circuit = QuantumCircuit(
            self._input_qr, self._qr, self._output_qr, self._cr02, self._cr13
        )
        meas_circuit.measure(self._input_qr[0], self._cr02[0])

        meas_circuit.rz(-self.theta, self._qr[0]).c_if(self._cr02, 0)
        meas_circuit.rz(self.theta, self._qr[0]).c_if(self._cr02, 1)
        meas_circuit.measure(self._qr[0], self._cr13[0])

        meas_circuit.rz(-self.phi, self._qr[1]).c_if(self._cr13, 0)
        meas_circuit.rz(self.phi, self._qr[1]).c_if(self._cr13, 1)
        meas_circuit.measure(self._qr[1], self._cr02[1])

        meas_circuit.rz(-self.lam, self._qr[2]).c_if(self._cr02, 0)
        meas_circuit.rz(self.lam, self._qr[2]).c_if(self._cr02, 1)
        meas_circuit.rz(self.lam, self._qr[2]).c_if(self._cr02, 2)
        meas_circuit.rz(-self.lam, self._qr[2]).c_if(self._cr02, 3)
        meas_circuit.measure(self._qr[2], self._cr13[1])
